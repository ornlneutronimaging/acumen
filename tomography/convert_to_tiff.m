...
mkdir(wdir,'fbp_images')
mkdir(wdir,'fbp_tv_images')
...
for j_slice = 1:nimage
    ...
    fid = fopen([wdir,'/reconstruction/fbp',num2str(js),'.dat'],'r');
    ...
    fid = fopen([wdir,'/reconstruction/fbp_tv',num2str(js),'.dat'],'r');
    fseek(fid,4*offset,-1);
    ...
    colormap gray
    drawnow
    imwrite(fbp_rec,[wdir,'/fbp_images/fbp',num2str(j_slice),'.tiff'],'tiff');
    imwrite(fbp_tv,[wdir,'/fbp_tv_images/fbp_tv',num2str(j_slice),'.tiff'],'tiff');
end
close all
